...
릴리즈가 나갈 때마다 QA 팀이 같은 시나리오를 처음부터 끝까지 수동으로 반복하고 있었다. 로그인 → 검색 → 장바구니 → 결제 → 취소. 매번 동일한 흐름을 사람이 직접 클릭하면서 확인한다.
...
- **E2E(최소 핵심만)** — 로그인, 결제, 주문, 취소 같은 **비즈니스 크리티컬 경로**만 자동화한다.
...
- QA는 **리스크 높은 영역**(결제, 정산, 인증 등)에만 수동 탐색 테스트를 집중한다.
...
- **Synthetic Monitoring(E2E 프로브)** — 로그인/결제 등 핵심 경로를 1분~5분 간격으로 점검한다.
