---
title: '좋은 AX 엔지니어란 무엇일까? 내가 놓치고 있던 것들'
description: '좋은 AX 엔지니어는 어떻게 일해야 하는지 고민하며 여러 코딩 에이전트 스킬과 하네스·평가 자료를 살펴봤다. 그 안의 원칙을 기준으로 지금 사용하는 외장 두뇌·자동화·다른 모델의 코드 리뷰를 살펴보고, 부족한 부분과 보완 순서를 정리했다.'
pubDate: 2026-08-05
category: 'workflow'
...
나는 평소에도 모델이나 프롬프트만큼 에이전트가 일하는 환경을 설계하는 일이 중요하다고 생각해왔다. 외장 두뇌를 만들고 자율 실행 루프를 붙인 과정도 여러 글에 정리했다. 이번에 확인하고 싶었던 것은 이 방향이 맞는지가 아니라, 실제로 만든 장치들이 제 역할을 하고 있는지였다.
...
flowchart TB
  ...
  Agent --> Result["작업 결과"]
  Result --> Verification["검증<br/>다른 모델의 diff 리뷰"]
...
**검증.** 커밋 전에는 코드를 작성한 에이전트가 아니라 다른 모델에게 diff 리뷰를 받는다. 코드를 작성한 모델은 이미 내린 판단을 그대로 따라가면서 같은 문제를 다시 놓칠 수 있다고 봤기 때문이다. 그래서 작성 과정에 참여하지 않은 모델에게 한 번 더 검토를 맡긴다.
...
먼저 [awesome-harness-engineering](https://github.com/ai-boost/awesome-harness-engineering)에 모인 자료를 보면서 에이전트 하네스가 다루는 범위를 정리했다. 계획과 작업 분해, 컨텍스트와 메모리, 도구와 권한, 실행과 오케스트레이션, 검증과 CI, 관찰과 추적, 디버깅과 평가까지 범위가 넓었다. 지금 사용하는 외장 두뇌와 자동화, 코드 리뷰 규칙도 모두 하네스의 일부였다. 하지만 각각을 하나의 작업 흐름으로 놓고 살펴본 적은 없었다.
...
TDD 스킬에는 같은 원칙이 다른 각도로 있다. 테스트를 쓰기 전에 어느 이음매에서 확인할지 먼저 적고 사람과 합의한다. 합의 안 된 이음매에는 테스트를 쓰지 않는다. 전부를 테스트할 수는 없으니 먼저 지킬 곳을 정하고 테스트를 핵심 경로에 집중하는 방식이다.
...
예시에 대한 지적도 같은 결이다. 규칙을 전부 적으려고 엣지 케이스를 잔뜩 밀어 넣는 게 흔한 실수이고, 대신 기대하는 동작을 잘 보여주는 대표 예시 몇 개를 고르라고 한다. 긴 규칙 목록보다 대표 예시 몇 개가 모델에 더 도움이 된다는 설명이다.
...
에이전트의 피드백에서는 무엇이 빠졌는지도 봐야 한다. 도구가 불편해도 직접 말하지 않고 해당 도구를 쓰지 않거나 다른 방법으로 돌아갈 수 있기 때문이다. 확인하려면 기록을 모아두고 직접 읽어야 한다.

직접 확인하지 않으면 예상과 다른 결과가 나올 수 있다. [AGENTS.md의 효과를 분석한 2026년 연구](https://arxiv.org/abs/2602.11988)에서는 저장소 지침 파일을 추가해도 해결률이 뚜렷하게 좋아지지 않았다. LLM이 만든 지침 파일에서는 비용만 20% 넘게 늘었다. 반면 기존 문서를 제거한 환경에서는 컨텍스트 파일이 도움이 된 결과도 있었다. 지침 파일의 유무보다 기존 문서에 없는 정보를 담고 있는지, 실제 작업에 도움이 되는지를 확인하는 일이 중요했다.

나도 Codex와 Claude에 전역 지침을 설정해두고, 프로젝트마다 필요한 규칙을 추가해서 사용한다. 하지만 지침이 있을 때와 없을 때 결과와 비용이 어떻게 달라지는지 직접 비교해본 적은 없다. 이 연구를 보면서 어떤 규칙을 전역에 두고 어떤 규칙을 프로젝트별로 나눌지, 실제 작업에 도움이 되지 않는 지침은 무엇인지 확인해봐야겠다고 생각했다.

---

## 현재 방식을 다시 살펴보니 문제 정의와 평가가 부족했다
...
**검증 기준을 뒤늦게 만든다.** 일을 맡길 때 무엇을 통과로 볼지 먼저 적지 않는다. 구현을 맡긴 뒤 결과를 보고 판단한다. 그러면 리뷰는 "이게 맞나"를 막연하게 확인하는 과정이 되고, 에이전트도 어디까지 해야 끝나는지 모른 채 작업을 시작한다. 이 순서는 통과와 실패를 가르는 방법부터 만들라는 디버깅 스킬의 첫 단계와 정반대다.

**평가 태스크를 만들지 않는다.** 스킬과 자동화를 여럿 만들었지만 평가 태스크로 비교해본 적이 없다. 몇 번 써보고 괜찮으면 그대로 뒀다. 호출 횟수와 토큰, 에러 빈도를 모르니 어느 스킬이 실제로 도움이 되는지 말할 근거도 없다. 도구는 만들었지만 평가 기준은 만들지 못했다.

**기록을 모으지 않는다.** 세션 로그와 도구 호출 기록을 남기는 장치가 없다. 잘 풀린 세션과 헤맨 세션이 뭐가 달랐는지 나중에 볼 방법이 없다. 빠져 있는 것을 읽으라는 조언은 기록이 있어야 쓸 수 있는 조언이다.

**지침의 효과를 비교하지 않는다.** 어떤 지침이 실제로 도움이 되고 어떤 지침이 컨텍스트와 비용만 늘리는지 알지 못한다.
...
**5. 전역·프로젝트 지침을 비교한다.** 자주 반복하는 작업을 골라 지침이 없을 때, 전역 지침만 적용했을 때, 프로젝트별 지침까지 적용했을 때를 비교한다. 결과 품질과 작업 시간, 토큰 사용량을 확인하고 전역에 둘 규칙과 프로젝트에만 둘 규칙을 다시 정리한다.

기록을 첫 순서로 둔 이유는 비교할 기준을 만들기 위해서다. 기록이 쌓이기 전에 스킬이나 규칙부터 바꾸면 무엇 때문에 결과가 달라졌는지 알기 어렵다. 지금은 장치를 더 추가하기보다 현재 상태를 기록하고 다음 변경과 비교할 수 있게 만드는 일이 먼저다.
