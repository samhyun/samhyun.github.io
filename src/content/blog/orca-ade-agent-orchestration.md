...
얼마 전까지 코딩 에이전트를 여러 개 굴릴 때 Superset을 썼다. 작업마다 git worktree를 만들어 브랜치를 격리하고, 에이전트가 끝낸 결과를 diff로 비교하는 흐름이 편했다. 파일 탐색기와 diff에서 코드를 직접 고칠 수 있고 내장 브라우저로 결과까지 확인할 수 있어서, 작은 수정이나 확인을 위해 별도의 에디터와 브라우저를 열지 않아도 된다는 점도 컸다.
...
며칠 사용해보니 둘의 차이는 코드 편집이나 브라우저의 유무보다 그다음 단계에 있었다. Superset에서 이미 사용하던 편집기와 브라우저 흐름에 UI 요소를 코드 위치와 연결하는 Design Mode, 모바일 시뮬레이터 제어, 예약 자동화와 Orca가 제공하는 에이전트 스킬이 더해졌다.
...
- **Superset**은 worktree 격리와 병렬 작업, 코드 편집, 브라우저 확인과 diff 검토를 한 작업 공간에서 관리한다.
- **Orca**는 이 흐름에 Design Mode, 자동화, 에이전트 스킬과 모바일 제어까지 연결한다.
...
flowchart TB
    ...
    L3["③ 워크플로 · Superset<br/>worktree · 편집기 · 브라우저 · 리뷰"]
    L4["④ 통합 환경 ADE · Orca<br/>Design Mode · 스킬 · 자동화 · 모바일"]
    L1 --> L2 --> L3 --> L4
```

*아래로 갈수록 한 앱이 책임지는 범위가 넓어진다. 다만 Superset도 편집기와 브라우저를 제공하고 cmux에도 diff 기능이 추가되는 식으로 서로 기능이 수렴하고 있어서 이 경계는 점점 흐려지는 중이다.*
...
Superset을 사용한 가장 큰 이유는 에이전트마다 git worktree를 자동으로 만들어주고, 그 안에서 코드 편집과 브라우저 확인까지 이어갈 수 있다는 점이었다.
...
Claude Code와 Codex를 비롯한 CLI 에이전트를 붙일 수 있고, 작업 공간별 터미널도 유지된다. 파일 탐색기에서 코드를 바로 열거나 diff의 원하는 위치에서 편집기로 넘어가 직접 수정할 수 있다. 작은 수정은 Superset 안에서 끝내고, IDE 기능이 필요한 작업은 VS Code와 Cursor, JetBrains, Xcode 같은 외부 에디터로 worktree를 바로 열 수 있었다.

내장 브라우저로 실행 결과를 앱 안에서 바로 확인하는 것도 가능하다.

그래도 제품의 중심은 분명하다. 여러 에이전트에게 작업을 나누고, 각 작업을 격리하고, 코드를 직접 고치며 결과를 확인하는 흐름이다. 에이전트 실행부터 편집과 브라우저 확인까지 한 작업 공간에서 이어가고 싶다면 잘 맞는 선택이었다.

---

## Orca에서 달라진 점

Orca도 기본 구조는 비슷하다. 작업마다 git worktree를 만들고 그 안에 에이전트 터미널과 편집기, 브라우저를 연결한다.

내 작업에서 차이가 크게 느껴졌던 부분은 그 뒤였다.

### 코드 편집과 diff 검토

Superset에서도 앱 안에서 코드를 직접 편집할 수 있었기 때문에 편집기 자체가 Orca로 옮긴 이유는 아니었다. Orca에서도 같은 흐름을 유지할 수 있다는 점이 오히려 전환 부담을 줄였다.
...
Orca는 CLI 기능을 에이전트가 사용할 수 있도록 공식 스킬로 제공한다. 현재 공개된 스킬은 worktree·터미널·브라우저·자동화를 제어하는 `orca-cli`, 여러 에이전트의 작업과 의사결정을 조율하는 `orchestration`, 데스크톱 앱을 조작하는 `computer-use`, Linear 이슈를 연결하는 `orca-linear`, iOS Simulator를 제어하는 `orca-emulator`다.

스킬을 설치하면 에이전트가 Orca의 명령과 사용 절차를 읽고 현재 작업에 맞는 기능을 직접 호출할 수 있다. 매번 worktree를 어떻게 만들고 브라우저나 시뮬레이터를 어떻게 제어할지 프롬프트에 다시 설명하지 않아도 됐다. 자동화가 정해진 프롬프트를 실행하는 기능이라면, 스킬은 그 작업을 수행하는 에이전트에게 Orca를 조작하는 방법을 건네는 역할에 가깝다.
...
물론 Superset의 모든 기능을 완벽하게 파악했다고 할 수는 없기 때문에 Orca가 더 낫다고 단정하기는 어렵다. 다만 실제로 사용해본 범위에서는 Orca로 에이전트를 더 다양한 방식으로 활용할 수 있었고, 여러 작업의 상태를 확인하거나 문제를 디버깅하는 과정에서도 장점이 있다고 느꼈다.

Superset에서도 worktree 격리와 병렬 에이전트 실행, 코드 편집, diff 검토와 브라우저 확인까지 한 앱에서 이어졌다. 별도의 에디터나 브라우저를 열지 않고 작업을 마칠 수 있다는 점은 Superset을 사용한 큰 이유였다.

Orca로 옮긴 이유는 이 기능들이 새로 필요해서가 아니었다. Superset에서 익힌 흐름을 유지하면서 자동화와 Orca 스킬로 에이전트가 직접 할 수 있는 작업의 범위를 넓힐 수 있다는 점이 컸다. 특히 세 가지가 옮기는 데 크게 작용했다.

1. 예약 자동화로 정해진 시간이나 조건에 맞춰 에이전트 작업을 시작할 수 있다.
2. `orca-cli`, `orchestration`, `computer-use`, `orca-emulator` 같은 공식 스킬로 에이전트가 Orca의 기능을 직접 사용할 수 있다.
3. Design Mode와 모바일 시뮬레이터 제어로 화면 확인과 수정 요청을 코드 작업에 바로 연결할 수 있다.

현재 Superset 공식 문서에도 예약 자동화 기능이 있다. 여기서 말하는 차이는 한쪽에만 기능이 있다는 뜻이 아니라, 내가 두 도구에서 실제로 사용한 범위와 Orca에서 자동화·스킬을 연결한 경험을 기준으로 한 것이다.

모바일 앱을 개발할 때 iOS Simulator를 worktree와 연결해 제어할 수 있다는 점도 컸다.

Superset에서 익힌 작업 방식은 거의 그대로 유지됐다. 편집기와 브라우저는 이미 익숙했고, 그 위에 자동화와 에이전트 스킬, Design Mode와 모바일 제어가 추가된 느낌에 가까웠다. 그래서 내 작업에서는 Orca가 더 잘 맞았다.
...
tmux는 가장 가볍고 통제할 수 있는 범위가 넓다. cmux는 터미널의 속도와 에이전트 알림에 강점이 있다. Superset은 worktree 격리부터 코드 편집, 브라우저 확인과 여러 에이전트의 결과 검토까지 한곳에서 이어진다.

Orca는 이 흐름에 자동화와 에이전트 스킬, Design Mode와 모바일 제어를 연결한다. 모든 사람에게 필요한 범위는 아닐 수 있다. worktree와 편집기, 브라우저를 한곳에서 사용하는 정도면 Superset의 범위가 잘 맞을 수도 있다.

나는 에이전트를 여러 worktree에 나눠 실행한 뒤 코드와 화면을 확인하고, 결과를 비교하고, 반복 작업을 자동화하는 흐름을 자주 쓴다. 에이전트가 Orca 스킬로 그 과정에 직접 참여할 수 있다는 점도 지금 Orca를 사용하는 이유다.
...
- [Superset Diff Viewer](https://docs.superset.sh/diff-viewer)
