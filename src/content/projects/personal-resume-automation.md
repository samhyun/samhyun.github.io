...
이력서를 쓸 때마다 비슷한 작업이 반복됐다. 경력을 정리하고 성과를 구체화하며 기술 스택을 맞추려면 이미 알고 있는 내용도 다시 찾아야 했다. PDF 이력서를 올리면 구조를 파악하고 부족한 내용을 질문한 뒤, 이력서 개선부터 자기소개서와 면접 준비까지 이어서 돕는 도구를 만들고 싶었다.
